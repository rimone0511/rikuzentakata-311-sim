...
# --- 震災後の嵩上げ地形の補正 ---
# 現在の標高タイルは震災後の嵩上げ(高田地区・今泉地区、約+10m)を含むため、
# 2011年当時の低地(約2〜3m)に近づける近似補正を行う。
# 正確な震災前DEM(基盤地図情報 旧版)への置換はフェーズ2(QUESTIONS.md 参照)。
KASAAGE_REGIONS = [
    # (lat_s, lat_n, lon_w, lon_e, 当時の推定標高)
    (39.011, 39.023, 141.616, 141.641, 2.8),  # 高田地区(中心市街地)
    (39.001, 39.011, 141.606, 141.621, 2.5),  # 気仙町今泉地区
]
KASAAGE_BAND = (4.5, 16.0)  # この帯域の標高だけ嵩上げとみなして下げる
SMOOTH_PASSES = 3           # 補正境界をなだらかにする平滑化回数
...
def main():
    ...
    def cell_latlon(gx, gy):
        px = x0 + (gx * DOWNSAMPLE + DOWNSAMPLE / 2) / 256.0
        py = y0 + (gy * DOWNSAMPLE + DOWNSAMPLE / 2) / 256.0
        return num2deg(px, py, Z5)
    ...
    for gy in range(H):
        for gx in range(W):
            ...
            for (s, n, w_, e, base) in KASAAGE_REGIONS:
                if s <= lat <= n and w_ <= lon <= e:
                    v = heights[gy * W + gx]
                    if lo < v < hi:
                        heights[gy * W + gx] = base
                        modified.add((gx, gy))
                    break
    # 補正セルとその近傍だけ平滑化して段差を緩和
    smooth_zone = set()
    for (gx, gy) in modified:
        for dy in range(-2, 3):
            for dx in range(-2, 3):
                if 0 <= gx + dx < W and 0 <= gy + dy < H:
                    smooth_zone.add((gx + dx, gy + dy))
    for _ in range(SMOOTH_PASSES):
        snapshot = heights[:]
        for (gx, gy) in smooth_zone:
            if gx < 1 or gy < 1 or gx >= W - 1 or gy >= H - 1:
                continue
            s = 0.0
            for dy in (-1, 0, 1):
                for dx in (-1, 0, 1):
                    s += snapshot[(gy + dy) * W + (gx + dx)]
            heights[gy * W + gx] = s / 9.0
    print(f"嵩上げ補正: {len(modified)} セルを当時の低地標高に変更")
